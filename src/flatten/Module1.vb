        ...
        While lstFolders.Count > 0
            ...
            If bBaseDone Then
                ...
                For Each item As String In files
                    lIndex = -1
                    Dim n As Integer = 0
                    If File.Exists(Path.Combine(sBaseFolder, Path.GetFileName(item))) Then
                        n = 1
                        While File.Exists(Path.Combine(sBaseFolder, Path.GetFileNameWithoutExtension(item) & "_" & n & Path.GetExtension(item)))
                            n += 1
                        End While
                    End If
                    ...
                    If n > 0 Then
                        File.Move(item, Path.Combine(sBaseFolder, Path.GetFileNameWithoutExtension(item) & "_" & n & Path.GetExtension(item)))
                    Else
                        File.Move(item, Path.Combine(sBaseFolder, Path.GetFileName(item)))
                    End If
                Next
